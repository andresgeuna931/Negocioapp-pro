...
import { Bell, Check, CheckCheck, Package, CreditCard, AlertTriangle } from 'lucide-react';
import {
...
function NotificationIcon({ type }: { type: string }) {
    switch (type) {
        case 'stock_low': return <Package className="w-4 h-4 text-amber-500" />;
        case 'payment_received': return <CreditCard className="w-4 h-4 text-emerald-500" />;
        case 'subscription_expiring': return <AlertTriangle className="w-4 h-4 text-orange-500" />;
